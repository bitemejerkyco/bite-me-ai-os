  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white/90 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className={`rounded-full border px-2 py-0.5 text-[11px] font-semibold uppercase tracking-[0.08em] ${priorityClasses[recommendation.roiPriority]}`}>
          {recommendation.roiPriority}
        </span>
      </div>
      <p className="mt-2 text-sm text-slate-700">{recommendation.why}</p>
      <div className="mt-3 grid gap-2 text-xs text-slate-600 md:grid-cols-2">
        <p><span className="font-semibold text-slate-700">Business impact:</span> {recommendation.businessImpact}</p>
        <p><span className="font-semibold text-slate-700">Expected outcome:</span> {recommendation.expectedOutcome}</p>
        <p><span className="font-semibold text-slate-700">Estimated effort:</span> {recommendation.estimatedEffort}</p>
        <p><span className="font-semibold text-slate-700">Approval:</span> {recommendation.approvalStatus.replaceAll("_", " ")}</p>
        <p><span className="font-semibold text-slate-700">Next workflow:</span> {recommendation.nextWorkflow}</p>
        <p><span className="font-semibold text-slate-700">Confidence:</span> {Math.round(recommendation.confidence * 100)}%</p>
      </div>
