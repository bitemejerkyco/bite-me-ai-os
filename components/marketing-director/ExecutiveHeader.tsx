    ...
    <section className="pm-glass rounded-[2rem] border border-white/90 bg-white/80 p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white/90 p-4 text-sm text-slate-700">
          ...
          <p className="mt-1 text-lg font-bold text-slate-900">{modeLabel[props.mode.operatingMode]}</p>
          <p className="mt-2 text-xs text-slate-500">{props.mode.autopilotMessage}</p>
          <Link href="/settings/marketing-director" className="mt-3 inline-flex rounded-full border border-slate-200 px-3 py-1 text-xs font-semibold text-violet-700 hover:bg-violet-50">
            Mode settings
          </Link>
        </div>
      </div>
